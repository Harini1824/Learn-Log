...
export default function CoordinatorStudentDetailPage() {
  ...
  return (
    <Layout>
      <div data-testid="page-coordinator-student-detail">
        <Button variant="ghost" className="mb-6" data-testid="btn-back" onClick={() => setLocation("/coordinator/students")}>
        ...
        {isLoading ? (
          ...
          <>
            <div className="mb-6">
              ...
              <div className="text-muted-foreground text-sm mt-1">{data.student.phone}{data.student.centreName ? ` · ${data.student.centreName}` : ""}</div>
              <div className="text-sm mt-1"><span className="font-medium">Course:</span> {data.student.courseName}</div>
              <div className="text-sm mt-0.5"><span className="font-medium">Completion:</span> <span className="text-primary font-semibold">{data.completionRate}%</span></div>
            </div>
                    ...
                    <div key={chapter.chapterId}>
                      ...
                      <div className="divide-y divide-border">
                        {chapter.modules.map(mod => (
                          <div key={mod.moduleId} className="px-5 py-3.5 flex items-center justify-between">
                            <div className="flex items-center gap-3">
                              {mod.rating ? <CheckCircle className="w-4 h-4 text-green-500 flex-shrink-0" /> : <div className="w-4 h-4 rounded-full border-2 border-muted-foreground/30 flex-shrink-0" />}
                              <div>
