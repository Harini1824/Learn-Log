...
export default function TrainerUpdatePage() {
  ...
  const listboxId = "trainer-student-listbox";
  const { data: students } = useListStudents({ trainerId: entityId ?? undefined, search: search || undefined });
  ...
  return (
    <Layout>
      <div data-testid="page-trainer-update">
        ...
        <div className="relative mb-8">
          ...
          <Input
            ...
            aria-activedescendant={highlightedIndex >= 0 ? `student-option-${dropdownItems[highlightedIndex]?.id}` : undefined}
          ...
          {dropdownItems.length > 0 && (
            <ul id={listboxId} role="listbox" aria-label="Students"
              ...
              {dropdownItems.map((s, idx) => (
                <li key={s.id} id={`student-option-${s.id}`} role="option" aria-selected={highlightedIndex === idx}
                        ...
                        <div key={mod.moduleId} data-testid={`module-update-${mod.moduleId}`} className="p-5">
                          ...
                          {ratings[mod.moduleId] && (
                            <Button data-testid={`btn-save-${mod.moduleId}`} type="button" size="sm" className="h-7 px-3 text-xs"
                              onClick={() => handleSave(mod.moduleId)} disabled={submitProgress.isPending}>
